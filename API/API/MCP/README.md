...
This project integrates the Model Context Protocol (MCP) to provide AI-enhanced analytics capabilities for our e-commerce application. The integration allows Claude and other MCP-compatible LLMs to interact with our data and perform advanced analytics operations.
...
1. Download and install [Claude Desktop](https://www.anthropic.com/claude-desktop) if you haven't already.
...
3. Add our server to the configuration:
    ...
    "mcpServers": {
        "api-server": {
            ...
            "args": [
                "run",
                "--project",
                "/ABSOLUTE/PATH/TO/API/API/API.csproj",
                "--no-build"
            ]
        }
    }
...
The following tools are available through our MCP server:

#### 1. Market Trend Analysis

- **Tool ID**: `phan_tich_xu_huong_san_pham`
- **Description**: Analyzes product sales trends over a specified period
- **Parameters**: 
  - `daysLookback` (default: 30): Number of days to look back
  - `topCount` (default: 10): Number of top items to return

#### 2. Seasonal Trend Analysis

- **Tool ID**: `phan_tich_thoi_vu`
- **Description**: Analyzes seasonal trends and patterns in sales data
- **Parameters**:
  - `monthsLookback` (default: 12): Number of months to analyze

#### 3. Product Correlation Analysis

- **Tool ID**: `phan_tich_tuong_quan`
- **Description**: Identifies products frequently purchased together
- **Parameters**:
  - `topCount` (default: 10): Number of top correlations to return
  - `daysLookback` (default: 90): Number of days to analyze

#### 4. Product Query

- **Tool ID**: `tim_san_pham`
- **Description**: Searches for products by name, category, or brand
- **Parameters**:
  - `query`: Search term

#### 5. Order Status Check

- **Tool ID**: `tra_cuu_don_hang`
...
- **Tool ID**: `goi_y_san_pham`
- **Description**: Recommends products based on user purchase history
- **Parameters**:
  - `userId`: ID of the user
  - `limit` (default: 5): Number of recommendations to return

#### 7. Similar Product Recommendations

- **Tool ID**: `goi_y_san_pham_tuong_tu`
- **Description**: Recommends products similar to a given product
- **Parameters**:
  - `productId`: ID of the reference product
  - `limit` (default: 5): Number of recommendations to return

#### 8. Review Analysis

- **Tool ID**: `phan_tich_danh_gia`
- **Description**: Analyzes product reviews to identify strengths and weaknesses
- **Parameters**:
  - `productId`: ID of the product to analyze

#### 9. Customer Sentiment Analysis

- **Tool ID**: `phan_tich_cam_xuc`
...
- **Tool ID**: `tao_mo_ta`
- **Description**: Generates product descriptions automatically
- **Parameters**:
  - `productId`: ID of the product
  - `style` (default: "marketing"): Style of the description to generate

#### 11. SEO Content Generation

- **Tool ID**: `tao_noi_dung_seo`
- **Description**: Creates SEO-optimized content in multiple languages
- **Parameters**:
  - `productId`: ID of the product
  - `language` (default: "vi"): Target language code

## API Testing Endpoints

For testing the MCP functionality, the following REST API endpoints are available:

- `GET /api/McpClient/list-tools`: Lists all available MCP tools
- `GET /api/McpClient/market-trends?daysLookback=30`: Gets market trend analysis
- `GET /api/McpClient/seasonal-trends?monthsLookback=12`: Gets seasonal trend analysis
- `GET /api/McpClient/product-correlations?topCount=10&daysLookback=90`: Gets product correlation analysis

## Example Usage in Claude

Once configured, you can use these tools directly in Claude Desktop. Here are some examples:

1. **Market Trend Analysis**
   ```
   Can you analyze the recent sales trends of our products in the last 60 days?
   ```

2. **Seasonal Analysis**
   ```
   Please identify any seasonal patterns in our sales over the past 12 months.
   ```

3. **Product Correlation**
   ```
   Which products are commonly purchased together based on our order data?
   ```

4. **Product Recommendations**
   ```
   What products would you recommend for user ID 12345?
   ```

5. **Review Analysis**
   ```
   Can you analyze the reviews for product ID 789 and summarize the main feedback?
   ```

## AI-Enhanced Features

When an LLM (like Claude) is available, the tools provide AI-enhanced analytics including:

- Detailed insights based on sales patterns
- Strategic recommendations for inventory and marketing
- Product bundle suggestions based on correlation analysis
- Sentiment analysis from customer reviews
- Automatic product description generation
- Multilingual SEO content creation

When no LLM is available, the tools fall back to basic statistical analysis. 
# Example Clients
